package effects;

import java.awt.Graphics;
import java.awt.geom.Point2D;
import java.awt.image.BufferedImage;
import java.util.Random;

import main.Game;
import utilz.LoadSave;

/**
 * Clase que representa el efecto de lluvia en el juego.
 */
public class Rain {

    private Point2D.Float[] drops;
    private Random rand;
    private float rainSpeed = 1.25f;
    private BufferedImage rainParticle;

    /**
     * Constructor de la clase Rain.
     * Inicializa la lluvia y carga la imagen de la partícula de lluvia.
     */
    public Rain() {
        rand = new Random();
        drops = new Point2D.Float[1000];
        rainParticle = LoadSave.GetSpriteAtlas(LoadSave.RAIN_PARTICLE);
        initDrops();
    }

    /**
     * Inicializa las posiciones de las gotas de lluvia.
     */
    private void initDrops() {
        for (int i = 0; i < drops.length; i++)
            drops[i] = getRndPos();
    }

    /**
     * Genera una posición aleatoria para una gota de lluvia.
     *
     * @return La posición generada.
     */
    private Point2D.Float getRndPos() {
        return new Point2D.Float((int) getNewX(0), rand.nextInt(Game.GAME_HEIGHT));
    }

    /**
     * Actualiza la posición de las gotas de lluvia.
     *
     * @param xLvlOffset El desplazamiento horizontal del nivel.
     */
    public void update(int xLvlOffset) {
        for (Point2D.Float p : drops) {
            p.y += rainSpeed;
            if (p.y >= Game.GAME_HEIGHT) {
                p.y = -20;
                p.x = getNewX(xLvlOffset);
            }
        }
    }

    /**
     * Genera una nueva posición en el eje X para una gota de lluvia.
     *
     * @param xLvlOffset El desplazamiento horizontal del nivel.
     * @return La posición generada en el eje X.
     */
    private float getNewX(int xLvlOffset) {
        float value = (-Game.GAME_WIDTH) + rand.nextInt((int) (Game.GAME_WIDTH * 3f)) + xLvlOffset;
        return value;
    }

    /**
     * Dibuja las gotas de lluvia en el lienzo del juego.
     *
     * @param g          El contexto gráfico en el que se dibujarán las gotas de lluvia.
     * @param xLvlOffset El desplazamiento horizontal del nivel.
     */
    public void draw(Graphics g, int xLvlOffset) {
        for (Point2D.Float p : drops)
            g.drawImage(rainParticle, (int) p.getX() - xLvlOffset, (int) p.getY(), 3, 12, null);
    }

}
