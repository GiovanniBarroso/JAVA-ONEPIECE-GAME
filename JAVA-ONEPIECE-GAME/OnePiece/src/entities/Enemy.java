...
public abstract class Enemy extends Entity {
	...
	public void hurt(int amount) {
	    ...
	    // Verificar si la vida actual es mayor que la vida máxima permitida para el tipo de enemigo
	    int maxHealth = GetMaxHealth(enemyType);
	    if (currentHealth > maxHealth) {
	        currentHealth = maxHealth; // Establecer la vida al máximo permitido
	    }

	    if (currentHealth <= 0&&enemyType!=KUROHIGE) {
	        newState(DEAD);
	    }
	    if (currentHealth <= 0&&enemyType==KUROHIGE) {
	        newState(DEAD3);
	    }
	    else if (currentHealth > 0 && enemyType == KUROHIGE) {
	        newState(HIT2);
